...
const withAuthorization = <T>(config: AxiosRequestConfig<T>, { isRefresh = false, force = false }: { isRefresh?: boolean, force?: boolean } = {}) => {
  config.headers ||= {};
    ...
    if (user?.id) {
      ...
      if (originalRequest.url !== refreshTokenURL) {
        return axios.get(refreshTokenURL, withAuthorization({}, { isRefresh: true })).then((refreshTokenResponse) => {
          ...
          history.replaceState(undefined, '', PAGES.LOGOFF);
          return Promise.reject(error);
        });
      }
    }

    // redirect user to logoff page
    history.replaceState(undefined, '', PAGES.LOGOFF);
  }
